...
def main():
    if len(sys.argv) == 1:
        ...
        print("        ou : ", prog, " -h")
        print(
            "example (liste): python %s -o liste -l /home/hagolle/LANDSAT liste_landsat8_site.txt -u usgs.txt" % sys.argv[0])
        print(
    ...
    if options.option == 'catalog':
        ...
        if produit.startswith('LC8'):
            ...
            collection_file = os.path.join(
                options.outputcatalogs, 'LANDSAT_8.csv')
        if produit.startswith('LE7'):
            repert = ['12267']
            collection_file = os.path.join(
                options.outputcatalogs, 'LANDSAT_ETM.csv')
        if produit.startswith('LT5'):
            repert = ['12266']
            if 2000 <= int(year_start) <= 2009:
                collection_file = os.path.join(
                    options.outputcatalogs, 'LANDSAT_TM-2000-2009.csv')
            if 2010 <= int(year_start) <= 2012:
                collection_file = os.path.join(
                    options.outputcatalogs, 'LANDSAT_TM-2010-2012.csv')
            ...
            while (curr_date < date_end) and check == 1:
                ...
                for station in stations:
                    for version in ['00', '01', '02']:
                        ...
                        if os.path.exists(lsdestdir):
                            print(
                                '   product %s already downloaded and unzipped' % nom_prod)
                            downloaded_ids.append(nom_prod)
                            check = 0
                        elif os.path.isfile(tgzfile):
                            print('   product %s already downloaded' %
                                  nom_prod)
                            if options.unzip != None:
                                p = unzipimage(nom_prod, rep_scene)
                                if p == 1 and options.clouds != None:
                            ...
                            try:
                                downloadChunks(url, "%s" %
                                               rep_scene, nom_prod+'.tgz')
                            except:
                                print('   product %s not found' % nom_prod)
                                notfound = True
                            if notfound != True and options.unzip != None:
