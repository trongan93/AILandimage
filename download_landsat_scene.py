...
def get_wrs_rest(lat, lng):
    url = "https://landsatlook.usgs.gov/arcgis/rest/services/LLook_Outlines/MapServer/1/query?where=MODE=%%27D%%27&geometry=%s,%%20%s&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=*&returnGeometry=false&returnTrueCurves=false&returnIdsOnly=false&returnCountOnly=false&returnZ=false&returnM=false&returnDistinctValues=false&f=json" % (lng, lat) 

    print(url)
    web_url = urllib.urlopen(url)
    data = web_url.read()
...
def main():
    if len(sys.argv) == 1:
	    ...
	    print "example (latlng): python %s -o latlng -d 20190801 -f 20190826 --latitude 37.234332396 --longitude -115.80666344 -u usgs.txt"%sys.argv[0]
	    print "example (latlng): python %s -z unzip -b LT5 -o latlng -d 20101001 -f 20101231 --latitude 37.234332396 --longitude -115.80666344 -u usgs.txt --output /outputdir/"%sys.argv[0]
	    print "example (latlng): python %s -z unzip -b LT5 -o latlng -d 20101001 -f 20101231 --latitude 37.234332396 --longitude -115.80666344 -u usgs.txt --output /outputdir/ -k update --outputcatalogs /outputcatalogsdir/"%sys.argv[0]		
	    print "example (latlng): python %s -b LE7 -o latlng -d 20151201 -f 20151231 --latitude 37.234332396 --longitude -115.80666344 -u usgs.txt --output . --dir=12267 --station SG1"%sys.argv[0]
	    sys.exit(-1)
    else:
        usage = "usage: %prog [options] "
        parser = OptionParser(usage=usage)
        parser.add_option("-o", "--option", dest="option", action="store", type="choice", \
			    help="latlng or liste or catalog", choices=['latlng','liste','catalog'],default=None)
        ...
        ### Location(latlng)/Time(yyyymmdd)/DatasetType(LC8, LE7, LT5)/Filter(CLOUD_COVER)
...
##########Telechargement des produits par latlng
            ...
            while (curr_date < date_end) and check==1:
                ...
                for station in stations:
                    for version in ['00','01','02']:			
                        nom_prod=produit+str(path).zfill(3)+str(row).zfill(3)+date_asc+station+version
                        tgzfile=os.path.join(rep_scene,nom_prod+'.tgz')
                        lsdestdir=os.path.join(rep_scene,nom_prod)				
